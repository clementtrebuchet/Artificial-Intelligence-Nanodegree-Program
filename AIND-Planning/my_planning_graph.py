...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # are the preconditions satisfied in the prev level
            if a_node.prenodes.issubset(self.s_levels[level]):
                # Add it to the set of action nodes
                la.add(a_node)
                # pre nodes
                for lit in self.s_levels[level]:
                    # connect node to the prev s literal level
                    if lit in a_node.prenodes:
                        lit.children.add(a_node)
                        a_node.parents.add(lit)

        # set the action level from action nodes
        self.a_levels.append(la)
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level - 1]:
            for lit in action_node.effnodes:  # get the effect node
                ll.add(lit)  # add effect to the  nodes set
                lit.parents.add(action_node)  # add the
                action_node.children.add(lit)

        # set the state level from effect nodes
        self.s_levels.append(ll)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # is node_a1 effects canceled by node_a2 neg effects ?
        for eff in node_a1.action.effect_add:
            if eff in node_a2.action.effect_rem:
                return True

        # is node_a2 effects canceled by node_a1 neg effects ?
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # is node_a1 effects is in opposition by node_a2 neg precond ?
        # is node_a2 effects is in opposition by node_a1 neg precond ?
        ...
        # is node_a1 effects is in opposition by node_a2  pos precond ?
        # is node_a2 effects is in opposition by node_a1  pos precond ?
        ...
        for g in self.problem.goal:
            ...
            for l in range(len(self.s_levels)):
                for s in self.s_levels[l]:
                    # literal symbol
                    ss = expr(s.symbol)
                    # if not positive, put the ~ symbol
                    if not s.is_pos:
                        ss = expr('~{}'.format(ss))
                    # state equal the goal ?
                    if g == ss:
                        gf = True
                        ls += l
                        break
